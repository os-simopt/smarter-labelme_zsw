...
subclasses = [{'artnichterkennbar', 'bussard', 'entenvogel', 'falke',  'greifvogel', 'grossvogel', 'habichtundsperber', 'kleinvogel', 'kolkrabe', 'kraehe', 'maeusebussard', 'milan', 'mittelgrosservogel', 'reiher', 'rotmilan',
'schwarzmilan', 'taube', 'turmfalke', 'vogel', 'wanderfalke', 'weihe'}, {'fluggeraet'}, {'vogeloderinsekt', "insekt"}]


def greaterThanOthers(l, idx):
    return l[idx]>=l[0] and l[idx]>=l[1] and l[idx]>=l[2]
...
def main():
    ...
    parser.add_argument('--datasize', type=int, help='how much data to create', default=250)
    ...
    for o in filelists:
        for f in filelists[o]:
            try:
                ...
                frame_id = re.search("([0-9]{8})_([0-9]{6})_([0-9]{5})", f)
                ...
                for shape in jdata['shapes']:
                    ...
                    for tc in dclasses:
                        if c.startswith(tc + ' '):
                            rc = dclasses[tc] # FIXME Klassen wie "artnichterkennbar" sind im temp file nicht ernthalten
                    if rc is not None:
                        shape['class'] = rc
    ...
    for dset in da:
                ...
                for anno in frame['useful_shapes']:
                    ...
                    if bbox[0] + bbox[2] >= offsetx and \
                        ...
                        if (bbox[2] > args.min_anno_size and bbox[3] > args.min_anno_size):
                            ...
                            for i in range(3): #loop is used to count annotations by subclass
                                if anno['class']['name'] in subclasses[i]:
                                    found_annotations += 1
                                    newanno = {}
                                    ...
                                    anno_delta[i] += 1.0
                                    newannos.append(newanno)
                                    annoid += 1
                                    break
            anno_count[0] += anno_delta[0]/float(found_annotations)
            anno_count[1] += anno_delta[1]/float(found_annotations)
            anno_count[2] += anno_delta[2]/float(found_annotations)

            jobs.append([newannos, frame, [offsetx, offsety, width, height]])


        #separate frames by class to increase chances of being picked in next step
        print("balancing dataset for %s" % dset)
        subframes = [[], [], []]
        for frameid in annotations[dset].keys():
            frame = annotations[dset][frameid]
            for anno in frame['useful_shapes']:
                for i in range(3):
                    if anno['class']['name'] in subclasses[i]:
                        subframes[i].append(frameid)



        cid = args.datasize if dset == 'train2017' else int(args.datasize * (args.test_percentage / 100))
        target_idx = anno_count.index(max(anno_count))

        #enhance each subclass' dataset till all subclasses have same number of annotations
        for i in range(3):
            if greaterThanOthers(anno_count, i):
                continue
            ...
            if not subframes[i]:
                if i == 0:
                    ...
                    sc = 'vogeloderinsekt'
                print("could not balance dataset for subclass %s, no data available" % sc)
                continue
            while not greaterThanOthers(anno_count, i):
                print('State:')
                print(anno_count)
                print('\n')
                found_annotations = 0
                ...
                newannos = []
                while found_annotations == 0:
                    frameid = rng.choice(subframes[i])
                    frame = annotations[dset][frameid]
                    width = rng.integers(frame['imageWidth'] - args.min_size) + args.min_size
                    height = int((width * args.height) / args.width)
                    if height > frame['imageHeight']:
                    ...
                    for anno in frame['useful_shapes']:
                        ...
                        if bbox[0] + bbox[2] >= offsetx and \
                            ...
                            if (bbox[2] > args.min_anno_size and bbox[3] > args.min_anno_size):
                                # valid annotation found - add to list
                                for j in range(3):  # loop is used to count annotations by subclass
                                    if anno['class']['name'] in subclasses[j]:
                                        found_annotations += 1
                                        newanno = {}
                                        newanno["area"] = bbox[2] * bbox[3]
                                        newanno["bbox"] = bbox
                                        ...
                                        annoid += 1
                                        break
                if max(anno_delta) != anno_delta[i]:
                    continue
                anno_count[0] += anno_delta[0] / float(found_annotations)
                anno_count[1] += anno_delta[1] / float(found_annotations)
                anno_count[2] += anno_delta[2] / float(found_annotations)
        ...
        ratio = float(args.datasize if dset == 'train2017' else int(args.datasize * (args.test_percentage / 100)))/float(len(jobs))
        print('Writing to folder')
        ctr = 0
        for job in jobs:
            if rng.random() < ratio:
                for anno in job[0]:
                    da[dset]['annotations'].append(anno)
                image = loadImage(job[1]['frame_folder'] + '/' + job[1]['imagePath'])
                newimage = scale(crop(image, job[2]), [args.width, args.height])
                imagefilename = ('%06d.jpg' % ctr)
                imagepath = args.destination_folder + '/' + dset + '/' + imagefilename
                saveImage(imagepath, newimage)
                newimagemeta = {'file_name': imagefilename, 'width': args.width, 'height': args.height, 'id': ctr}
        ...
        if dset == 'train2017' and args.cocofolder != "":
            ...
            for img in tqdm(cocoannos['images']):
                nimg = copy.copy(img)
                ciid = img["id"]
                cid += 1
