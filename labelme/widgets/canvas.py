...
CURSOR_ROT = QtCore.Qt.SizeAllCursor
...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.auxpixmap = QtGui.QPixmap()
        self.blendpixmap = QtGui.QPixmap()
        self.aux_image_opacity = 0.5
        self.visible = {}
        ...
        self.autocontour = AutoContour(self)
    ...
    def boundedRotateShapes(self, shape, pos):
        ...
        if dp.x() != 0 and dp.y() != 0:
            '''
            p1 = pos - cp
            p0 = self.prevPoint - cp
            
            t1 = np.arctan2(p1.x(),p1.y())
            if t1 < 0:
                t1 = np.pi + t1
            
            t0 = np.arctan2(p0.x(),p0.y())
            if t0 < 0:
                t0 = np.pi + t0
    ...
    def paintEvent(self, event):
        ...
        _p = QtGui.QPainter()
        _p.begin(self.blendpixmap)
        _p.drawPixmap(0, 0, self.pixmap)
        
        if not self.auxpixmap.isNull():
            _p.setOpacity(self.aux_image_opacity)
            _p.drawPixmap(0, 0, self.auxpixmap)
    ...
    def keyPressEvent(self, ev):
        ...
        if key == QtCore.Qt.Key_Escape and self.current:
            ...
            status = self.removeSelectedPoint()
            if status:
                self.update()
